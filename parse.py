...
#     return return_string
    ...
    with open("pci.ids.hpp", 'w+') as output:
        output.write("""#ifndef PCI_IDS_HPP
#define PCI_IDS_HPP
#include <string>
#include <array>

inline std::array<std::string, %s> get_pci_vendors_array() {
    return %s;
}

inline std::array<int, %s> get_pci_vendors_location_array() {
    return %s;
}
...
inline const std::array<std::string, %s> pci_vendors_array = get_pci_vendors_array();
inline const std::array<int, %s> pci_vendors_location_array = get_pci_vendors_location_array();
#endif  // PCI_IDS_HPP""" % (len(vendor_array), repr(vendor_array).replace("'", '"').replace('[', '{').replace(']', '}'), len(location_array), repr(location_array).replace("'", '"').replace('[', '{').replace(']', '}'), f.read(), len(vendor_array), len(location_array)))
