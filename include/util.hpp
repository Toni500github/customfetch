...
void error(const std::string_view fmt, Args&&... args) noexcept{
    fmt::println(stderr, BOLD_COLOR(fmt::rgb(fmt::color::red)), "ERROR: {}", fmt::format(fmt::runtime(fmt), std::forward<Args>(args)...));
}
...
#if DEBUG
    fmt::println(BOLD_COLOR((fmt::rgb(fmt::color::hot_pink))), "[DEBUG]: {}", fmt::format(fmt::runtime(fmt), std::forward<Args>(args)...));
#endif
}

template <typename... Args>
void warn(const std::string_view fmt, Args&&... args) noexcept{
    fmt::println(BOLD_COLOR((fmt::rgb(fmt::color::yellow))), "WARNING: {}", fmt::format(fmt::runtime(fmt), std::forward<Args>(args)...));
}
