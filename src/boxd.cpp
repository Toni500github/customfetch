...
static std::string strip_ansi(const std::string& str)
{
    ...
    for (size_t i = 0; i < str.size(); ++i)
    {
        ...
        if (in_escape)
        {
            ...
            {  // Terminal codes end with a letter, usually 'm'
                in_escape = false;
            }
            continue;
        }
...
std::string process_room_line(std::string line, const Config& config, const RoomLayoutInfo& layout_info)
{
    ...
    if (pin_pos != std::string::npos)
    {
        ...
        size_t right_padding                = (layout_info.total_width > current_line_width_if_packed)
                                                  ? (layout_info.total_width - current_line_width_if_packed)
                                                  : 0;
    ...
    if (fill_count > 0)
    {
        ...
        size_t total_fill_needed =
            (layout_info.total_width > static_width) ? (layout_info.total_width - static_width) : 0;
        ...
        for (size_t i = 0; i < fill_count; ++i)
        {
            ...
            if (current_fill_pos == std::string::npos)
