...
std::string name_from_id(const std::string& pci_ids, const std::string& id_str) {
    ...
    // Step 2: Find the position of the opening square bracket after the ID
    size_t bracket_open_pos = pci_ids.find('[', id_pos);
    if (bracket_open_pos == std::string::npos)
        return UNKNOWN; // Opening bracket not found

    // Step 3: Find the position of the closing square bracket after the opening bracket
    size_t bracket_close_pos = pci_ids.find(']', bracket_open_pos);
    if (bracket_close_pos == std::string::npos)
        return UNKNOWN; // Closing bracket not found

    // Step 4: Extract the substring between the brackets
    return pci_ids.substr(bracket_open_pos + 1, bracket_close_pos - bracket_open_pos - 1);
}
